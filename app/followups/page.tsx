...
export default async function FollowupsPage() {
  ...
  const { data: prospects } = await supabase
    .from('prospects')
    .select('*')
    .in('status', ['demo_generated', 'followup_r2'])
    ...
    <div className="p-8 max-w-2xl">
      <div className="mb-8 animate-fade-in-up">
        ...
        <p className="text-sm mt-1" style={{ color: '#94a3b8' }}>
          Site démo envoyé sans réponse depuis 7j · En relance R2 sans réponse depuis 5j
        </p>
      </div>
      ...
      {toFollowUp.length === 0 ? (
        ...
        <div className="space-y-3">
          {toFollowUp.map((p, i) => {
            const ref = p.last_contact_at ?? p.updated_at
            const daysSince = daysSinceUpdated(ref, now)
            const isUrgent = daysSince >= 14
            const statusLabel = p.status === 'demo_generated' ? 'Site démo envoyé' : 'En relance R2'
            return (
              <div key={p.id}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-0.5 font-medium"
                      ...
                      {statusLabel} · {daysSince} jours sans réponse
                    </p>
                  </div>
                </div>
